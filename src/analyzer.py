...
def call_openrouter(messages: list[dict]) -> str:
    response = requests.post(
        ...
        json={
            ...
            "max_tokens": 4000,
            "temperature": 0.1,
            # cobuddy has reasoning on by default. Under a fixed max_tokens
            # budget it was sometimes spending the whole thing on hidden
            # reasoning and leaving nothing for the actual answer — seen in
            # production as both flat-out empty responses AND responses
            # that parsed to "" after markdown-fence stripping (the
            # "Expecting value: line 1 column 1" errors). This task needs a
            # structured JSON answer, not visible chain-of-thought, so turn
            # reasoning off entirely rather than trying to budget around it.
...
def analyze_file(file: dict, repo_context: dict) -> list[dict]:
    ...
    try:
        ...
        if not findings:
            print("[Analyzer]   → 0 finding(s) above confidence threshold.")
            return []
